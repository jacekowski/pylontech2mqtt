...
class Device(dict):
    def __init__(self, identifiers, name, display_name, sw_version, model, manufacturer):
        super().__init__()
...
class Sensor(Component):
    def __init__(
        ...
        self.topic = f"{self.parent_device.mqtt_name}/{self.component}/{self.topic_parent_level}/{self.object_id}"
        self._send_config()

    def _send_config(self):
        _config = {
            "unique_id": self.parent_device["identifiers"]+"_"+self.object_id,
            "state_topic": self.topic+"/state",
            "name": self.parent_device.name+" "+self.name,
        ...
        uniq_id = self.parent_device["identifiers"]
        self.client.publish(
            f"{DISCOVERY_PREFIX}/{self.component}/{self.parent_device.mqtt_name}{uniq_id}/{self.object_id}/config",
            json.dumps(_config),
            retain=True,
        ).wait_for_publish()
